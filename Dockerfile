...
RUN \
    ...
    mkdir -p /run/dbus &&        mkdir -p /opt/workspace-internal/ && \
    # Clone the ARENA 3.0 repository
    git clone --depth 1 https://github.com/callummcdougall/ARENA_3.0.git /opt/workspace-internal/ARENA_3.0
...
RUN \
    ...
    mkdir -p /venv && \
    if [[ -d /opt/conda ]]; then \
    # Nvidia base image with Conda
    # Symlink /opt/conda to /opt/miniforge3 for compatibility
    ln -s /opt/conda /opt/miniforge3 && \
    # Ensure user can write to conda
    chown -R user:root /opt/conda && \
    chmod -R g+w /opt/conda && \
    # Create /venv/conda inheriting from base
    /opt/conda/bin/conda create --clone base -p /venv/conda && \
    # Create /venv/uv inheriting from base (system-site-packages)
    uv venv --python /opt/conda/bin/python --system-site-packages /venv/uv; \
    else \
    # Fallback for non-nvidia images or nvidia images without Conda
    ...
    /opt/miniforge3/bin/conda create -p /venv/conda python="${PYTHON_VERSION}" -y && \
    if [[ "$BASE_IMAGE" == *"nvidia"* ]]; then \
    # For Nvidia images without Conda, use system python for uv to inherit base packages (like pytorch)
    uv venv --python python3 --system-site-packages /venv/uv; \
    else \
    # Otherwise use the fresh conda python
    uv venv --python /opt/miniforge3/bin/python /venv/uv; \
    fi; \
    fi && \
    # Symlink /venv/main to /venv/conda for compatibility
    ln -s /venv/conda /venv/main && \
    # Setup activation scripts for /venv/conda (which is /venv/main)
    mkdir -p /venv/conda/etc/conda/{activate.d,deactivate.d} && \
    printf '%s\n' \
    'echo -e "\033[32mActivated conda virtual environment at \033[36m$(realpath $CONDA_PREFIX)\033[0m"' \
    >> /venv/conda/etc/conda/activate.d/environment.sh && \
    # Create standard activate script
    printf '%s\n' \
    ...
    '        if [[ -f /opt/miniforge3/etc/profile.d/conda.sh ]]; then source /opt/miniforge3/etc/profile.d/conda.sh; fi' \
    '    fi' \
    'fi' \
    '' \
    'conda activate $(realpath /venv/conda)' \
    > /venv/conda/bin/activate && \
    # Clean up
    if [[ -d /opt/miniforge3 ]]; then /opt/miniforge3/bin/conda clean -ay; fi

COPY arena_requirements.txt /tmp/arena_requirements.txt
...
RUN \
    ...
    ipykernel \
    ipywidgets && \
    # Install Arena 3.0 requirements
    uv pip install -r /tmp/arena_requirements.txt && \
